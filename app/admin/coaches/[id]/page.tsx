  ...
  useEffect(() => {
    const fetchCoach = async () => {
      try {
        ...
        const response = await fetch(`/api/coaches/${coachId}?t=${timestamp}`, { 
          cache: 'no-store', 
          next: { revalidate: 0 } 
        })
        if (response.ok) {
        ...
        setError("Koç yüklenirken bir hata oluştu")
      } finally {
        setFetching(false)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || "Koç güncellenirken bir hata oluştu")
      }
    } catch (err) {
      setError("Koç güncellenirken bir hata oluştu")
    } finally {
      setLoading(false)
    }
  }
      ...
      <section className="relative overflow-hidden border-b border-[#0b0b0b]/5 bg-gradient-to-b from-white via-[#f4ecde] to-[#f7f4ec]">
        ...
        <div className="mx-auto max-w-6xl px-4 py-16 md:px-6 md:py-20">
          <SectionHeading
            ...
            eyebrow="Koç Düzenle"
            title="Koçu Güncelle"
            subtitle="Mevcut koç bilgilerini düzenleyin"
          />
        </div>
      </section>
